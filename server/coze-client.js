...
const WORKFLOW_ID_CONFLICTS_WITH_BOT = Boolean(WORKFLOW_ID && BOT_ID && WORKFLOW_ID === BOT_ID);
...
function getCozeStatus() {
  return {
    ...
    workflowConfigured: Boolean(WORKFLOW_ID) && !WORKFLOW_ID_CONFLICTS_WITH_BOT,
    tokenConfigured: Boolean(PAT_TOKEN),
    workflowIdNeedsConfig: WORKFLOW_ID_CONFLICTS_WITH_BOT,
...
function assertCozeConfig(names) {
  const missing = names.filter(name => {
    ...
    if (name === 'COZE_WORKFLOW_ID') return !WORKFLOW_ID || WORKFLOW_ID_CONFLICTS_WITH_BOT;
